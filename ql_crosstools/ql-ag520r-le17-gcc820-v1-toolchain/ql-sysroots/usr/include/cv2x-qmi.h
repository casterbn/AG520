...
#define V2X_IP_INTERFACE "rmnet_data14"
#define V2X_NON_IP_INTERFACE "rmnet_data15"
#define MAX_INTERFACE_NAME (IFNAMSIZ)
#define MAX_V2X_SUB QMI_WDS_V2X_MAX_SUB_V01
#define CV2X_MAX_RX_POOL_LIST 4
#define CV2X_MAX_TX_POOL_LIST 2

/* Bit mask for dsi intialization mode */
const dsi_init_mode_t DSI_INIT_MODE = DSI_MODE_LOW_LATENCY_EX;
...
typedef struct cv2x_state {
    ...
    /* Non-shared objects */
    data_call_info_t data_call[2]; // Index corresponds to TRAFFIC_IP_TYPE enum
...
int bind_mux_data_port(qmi_client_type client, data_call_info_t *data_call);
...
qmi_client_type get_wds_client_by_interface(traffic_ip_type_t type, cv2x_state_t *state);
