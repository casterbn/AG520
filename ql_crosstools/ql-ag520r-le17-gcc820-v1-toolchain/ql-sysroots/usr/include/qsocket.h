...
*/
/*=========================================================================*/
static inline int qsendto(int fd, const void *buf, size_t len, int flags,
                          const struct qsockaddr *addr, qsocklen_t addrlen)
{
  int ret;
...
{
  int ret;
  ret = recvfrom(fd, buf, len, flags,  (struct sockaddr *)addr,
                 (socklen_t *)addrlen);
  if (ret < 0)
    return -errno;
